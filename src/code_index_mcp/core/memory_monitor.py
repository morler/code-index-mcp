...
class MemoryThreshold:
    ...
    def check_warning(self, current_mb: float, max_mb: float) -> bool:
        ...
        ) or current_mb > self.backup_limit_mb * (self.warning_percent / 100)

    def check_critical(self, current_mb: float, max_mb: float) -> bool:
        """Check if critical threshold exceeded"""
        return current_mb > max_mb * (
            self.critical_percent / 100
        ) or current_mb > self.backup_limit_mb * (self.critical_percent / 100)
...
class MemoryMonitor:
    ...
    def _check_limits_without_update(
        self, operation_name: str = "backup"
...
def default_alert_handler(level: str, alert_data: Dict[str, Any]) -> None:
    ...
    prefix = {"warning": "⚠️  MEMORY WARNING", "critical": "🚨 MEMORY CRITICAL"}.get(
        level, "📊 MEMORY ALERT"
    )
