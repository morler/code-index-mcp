...
class FileLock:
    ...
    def __init__(self, 
        ...
        if self.lock_type not in ('exclusive', 'shared'):
            raise ValueError(f"Invalid lock type: {self.lock_type}. Must be 'exclusive' or 'shared'")
            ...
            if file_key in self._locks:
                ...
                if existing_lock.is_locked():
                    if existing_lock.lock_type == lock_type or lock_type == 'shared':
                        ...
                        raise LockAcquisitionError(f"File already locked with incompatible type: {existing_lock.lock_type}")
                else:
                    # Remove stale lock
...
def acquire_file_lock(file_path: Union[str, Path], 
                     lock_type: str = 'exclusive',
                     timeout: float = 30.0) -> FileLock:
...
def is_file_locked(file_path: Union[str, Path]) -> bool:
    ...
    try:
        lock = FileLock(file_path, timeout=0.1)
