...
class SCIPSymbolManager:
    ...
    def generate_symbol_id(self, symbol_name: str, file_path: str, 
                          language: str, symbol_type: str = "unknown") -> str:
        """
        生成SCIP标准符号ID
    ...
    def resolve_references(self, symbol_id: str) -> List[SCIPOccurrence]:
        ...
        for document in self.documents.values():
            for occurrence in document.occurrences:
                if (occurrence.symbol_id == symbol_id and 
                    occurrence.occurrence_type == "reference"):
    ...
    def resolve_definitions(self, symbol_id: str) -> List[SCIPOccurrence]:
        ...
        for document in self.documents.values():
            for occurrence in document.occurrences:
                if (occurrence.symbol_id == symbol_id and 
                    occurrence.occurrence_type in ["definition", "declaration"]):
    ...
    def process_file_symbols(self, file_path: str, language: str, 
                           symbols: List[Dict[str, Any]]) -> SCIPDocument:
        """
        处理文件符号 - 语言无关统一接口
        ...
        # 获取语言处理器
        processor = self._language_processors.get(language, self._process_generic_symbol)
    ...
    def find_cross_references(self, symbol_name: str) -> Dict[str, List[SCIPOccurrence]]:
        """
        查找符号的跨文件引用
    ...
    def get_symbol_graph(self, symbol_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "cross_file_usage": len(set(
                occ.file_path for occ in self.resolve_references(symbol_id)
            ))
        }
    ...
    def export_scip_index(self) -> Dict[str, Any]:
        ...
        return {
                    ...
                    "occurrences": [
                        {
                            ...
                            "symbol_roles": self._map_occurrence_type_to_roles(occ.occurrence_type)
                        }
            ...
            "external_symbols": list(set().union(*[
                doc.external_symbols for doc in self.documents.values()
            ]))
        }
    
    # 私有方法 - 语言特定处理器
    
    def _process_python_symbol(self, symbol_data: Dict[str, Any], 
                              file_path: str, language: str) -> Optional[SCIPSymbol]:
        """Python符号处理器"""
    ...
    def _process_javascript_symbol(self, symbol_data: Dict[str, Any],
                                  file_path: str, language: str) -> Optional[SCIPSymbol]:
        """JavaScript符号处理器"""
        # JavaScript特定逻辑
        return self._process_generic_symbol(symbol_data, file_path, language)
    
    def _process_typescript_symbol(self, symbol_data: Dict[str, Any],
                                  file_path: str, language: str) -> Optional[SCIPSymbol]:
        """TypeScript符号处理器"""
        # TypeScript特定逻辑，包含类型信息
        return self._process_generic_symbol(symbol_data, file_path, language)
    
    def _process_java_symbol(self, symbol_data: Dict[str, Any],
                            file_path: str, language: str) -> Optional[SCIPSymbol]:
        """Java符号处理器"""
        # Java特定逻辑，包含包名和访问修饰符
        return self._process_generic_symbol(symbol_data, file_path, language)
    
    def _process_go_symbol(self, symbol_data: Dict[str, Any],
                          file_path: str, language: str) -> Optional[SCIPSymbol]:
        """Go符号处理器"""
        # Go特定逻辑，包含包和接口信息
        return self._process_generic_symbol(symbol_data, file_path, language)
    
    def _process_zig_symbol(self, symbol_data: Dict[str, Any],
                           file_path: str, language: str) -> Optional[SCIPSymbol]:
        """Zig符号处理器"""
        # Zig特定逻辑
        return self._process_generic_symbol(symbol_data, file_path, language)
    
    def _process_generic_symbol(self, symbol_data: Dict[str, Any],
                               file_path: str, language: str) -> Optional[SCIPSymbol]:
        """通用符号处理器 - 语言无关逻辑"""
        symbol_id = self.generate_symbol_id(
            symbol_data['name'], 
    ...
    def _create_scip_range(self, line: int, column: int, end_line: int = None, 
                          end_column: int = None) -> List[int]:
