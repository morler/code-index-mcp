...
class AdvancedQueryCache:
    ...
    def get_query_result(
        self, query: SearchQuery, index: CodeIndex
    ...
    def cache_query_result(
        self, query: SearchQuery, result: SearchResult, index: CodeIndex
    ):
        """缓存查询结果 - 记录文件依赖"""
        cache_key = self._generate_cache_key(query)
        ...
        for file_path in dependencies:
            self._file_signatures[file_path] = self._calculate_file_signature(
                file_path, index
            )
    ...
    def _extract_file_dependencies(
        ...
        if query.type in ["symbol", "references", "definition", "callers"]:
            ...
            for file_path in index.files.keys():
                if query.file_pattern is None or self._matches_pattern(
    ...
    def _estimate_memory_usage(self) -> str:
        ...
        deps_size = sum(
            len(deps) * 50 for deps in self._file_dependencies.values()
        )  # 依赖关系
