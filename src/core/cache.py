...
try:
    from .io_optimizer import read_file_lines_optimized
...
class OptimizedFileCache:
    ...
    def __init__(
        self, max_size: Optional[int] = None, max_memory_mb: Optional[int] = None
    ):
    ...
    def get_file_lines(self, file_path: str) -> List[str]:
        ...
        self._access_counts[normalized_path] = (
            self._access_counts.get(normalized_path, 0) + 1
        )
    ...
    def _emergency_cleanup(self) -> None:
        ...
        if len(self._cache) - len(files_to_remove) > target_size:
            ...
            files_to_remove.extend(
                [f[0] for f in files_with_scores[:additional_removes]]
            )
    ...
    def _calculate_pattern_score(
        self, recent_accesses: List[float], current_time: float
    ) -> float:
        """计算访问模式评分 - 规律访问获得更高分"""
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_requests_per_hour": round(
                self._total_requests / max(uptime_hours, 0.01), 2
            ),
            # 系统内存
            ...
            result.append(
                {
                    "file": file_path,
                    "access_count": count,
                    "last_access_ago_minutes": round(
                        (time.time() - last_access) / 60, 1
                    ),
                }
            )
