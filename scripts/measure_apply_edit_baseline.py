...
def measure_apply_edit_performance(file_size_kb: int = 10) -> Dict[str, Any]:
    ...
    try:
        ...
        test_result = apply_edit("/tmp/test", "test", "test")
        if test_result.get("error") == "MCP not available":
            print("MCP not available, using direct edit operations")
            # Use direct edit operations
            from core.edit_operations import get_memory_edit_operations

            def apply_edit(
                file_path: str, old_content: str, new_content: str, **kwargs
            ):
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Measure apply_edit performance baseline"
    )
    parser.add_argument(
        "--sizes",
        type=str,
