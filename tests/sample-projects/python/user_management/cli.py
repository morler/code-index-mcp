...
@click.option(
    "--role",
    type=click.Choice(["user", "admin", "guest"]),
    default="user",
...
def create_user(
    ...
    try:
        ...
        user = user_manager.create_user(
            name=name, username=username, age=age, email=email, role=user_role
        )
...
def list_users(ctx):
    ...
    for user in users:
        ...
        click.echo(
            f"{status_indicator} {user.username:<15} {user.name:<20} {user.role.value:<10} {user.email or 'No email'}"
        )
...
def search(ctx, query: str):
    ...
    for user in users:
        status_indicator = "✓" if user.is_active() else "✗"
        click.echo(
            f"{status_indicator} {user.username:<15} {user.name:<20} {user.role.value:<10}"
        )
