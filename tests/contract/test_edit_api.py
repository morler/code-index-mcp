...
class TestEditAPIContract:
    ...
    def test_edit_success_contract(self, temp_project):
        ...
        assert file_path.read_text() == new_content, (
            "File content should match new content"
        )
    ...
    def test_edit_not_found_contract(self, temp_project):
        ...
        # Apply edit
        success, error = apply_edit_with_backup(
            request["file_path"], request["new_content"]
        )

        # Expected error response contract (400 Bad Request)
        expected_error_response = {
            "error": "File not found",
            "details": {"file_path": str(non_existent_file)},
        }
    ...
    def test_edit_content_mismatch_contract(self, temp_project):
        ...
        request = {"file_path": str(file_path), "new_content": new_content}
        ...
        if success:
            ...
            assert file_path.read_text() == new_content, (
                "File should be edited if success"
            )
            print("✅ Content mismatch handled gracefully (content adapted)")
        else:
            # If it failed, verify error is appropriate
            assert "Content" in error or "validation" in error, (
                f"Error should mention content: {error}"
            )
    ...
    def test_edit_large_file_contract(self, temp_project):
        ...
        expected_error_response = {
            ...
            "details": {"max_size_mb": 10, "actual_size_mb": 11},
        }

        # Verify contract
        assert not success, "Edit should fail for large file"
        assert "large" in error.lower() or "memory" in error.lower(), (
            f"Error should mention size: {error}"
        )
    ...
    def test_edit_memory_status_contract(self, temp_project):
        ...
        # Verify data types
        assert isinstance(status["memory"]["current_mb"], (int, float)), (
            "Memory current should be number"
        )
        assert isinstance(status["backups"]["backup_count"], int), (
            "Backup count should be integer"
        )
        assert isinstance(status["limits"]["max_memory_mb"], (int, float)), (
            "Max memory should be number"
        )
    ...
    def test_edit_operation_status_contract(self, temp_project):
        ...
        if backup_info:
            ...
            assert backup_info["status"] in valid_statuses, (
                f"Status should be valid: {backup_info['status']}"
            )
